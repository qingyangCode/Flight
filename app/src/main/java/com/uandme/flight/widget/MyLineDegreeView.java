...
//	};
	...
	private Path createBottomRightAngle(){
		...
		path.lineTo(bottomRight.x - angleLeftLength ,  bottomRight.y - angleTopLength);
		path.lineTo(bottomRight.x - angleLeftLength ,  bottomRight.y + angleTopLength);
		path.close();
		return path;
	}
	...
	private void drawXText(Canvas canvas, Paint paint){
		...
		canvas.drawText("(" + LimitminX+")", getBottomLeft().x ,getBottomLeft().y+50, paint);
		canvas.drawText("(" + LimitmaxX+")", getBottomRight().x  ,getBottomRight().y+50, paint);
	}
	
	private void drawYText(Canvas canvas, Paint paint){
		paint.setColor(textColor);
		canvas.drawText("("+LimitminY+")", getBottomLeft().x -50,getBottomLeft().y, paint);
		canvas.drawText("(" + LimitmaxY+")", getTopLeft().x - 50 ,getTopLeft().y , paint);
	}
	
	
	private boolean bShowXGrid = true;
	private boolean bShowYGrid = false;
	...
	private void drawYGrid(Canvas canvas, Paint paint){
		...
		for(int i=1;i<yGridNum;i++){
			...
			canvas.drawLine(getBottomLeft().x, getBottomLeft().y - h, getBottomRight().x, getBottomLeft().y - h, paint);
		}
	}
	...
	private void drawXGrid(Canvas canvas, Paint paint){
		...
		for(int i=1;i<xGridNum;i++){
			...
			canvas.drawLine(getBottomLeft().x + h, getBottomLeft().y, getBottomLeft().x + h, getTopLeft().y, paint);
		}
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawLine(getBottomLeft().x, getBottomLeft().y, getBottomRight().x, getBottomRight().y, paint);
		canvas.drawLine(getBottomLeft().x, getBottomLeft().y, getTopLeft().x, getTopLeft().y, paint);
	...
	private Path getPathFromLineData(LineData lineData){
		...
		if(lineData.datasY!= null && lineData.datasX != null && lineData.datasX.size() >0){
